...
tools\3dstool -cvt0167f cci build\0.cxi build\1.cfa build\6.cfa build\7.cfa "Stella Glow (JPN) (zh).3ds" --header cci\ncsdheader.bin
IF NOT "%~1"=="" (
tools\3dstool --diff -vt cci --old %1 --new "Stella Glow (JPN) (zh).3ds" --patch-file patch.3ps
tools\icn2ico cci\cxi0\exefs\icon.icn zh\icon.ico
CALL make_3ps_exe.bat zh\icon.ico patch.3ps BS3JTOOLS || CALL ..\exepatch\make_3ps_exe.bat zh\icon.ico patch.3ps BS3JTOOLS
DEL zh\icon.ico
)
CD /D "%cwdir%"
PAUSE
